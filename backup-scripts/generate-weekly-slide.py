...
Design: fond ASM officiel + sidebar navy + tableau professionnel
"""
...
NAVY    = rgb("1B2B4B"); WHITE   = rgb("FFFFFF"); SILVER  = rgb("F0F4F8")
BORDER  = rgb("CBD5E1"); TEXT    = rgb("1E293B"); SUBTEXT = rgb("475569")
AMBER   = rgb("D4900A"); RED_ACC = rgb("C0392B"); BLUE_ACC= rgb("1E6BB5")
CRIMSON = rgb("991B1B"); ORANGE  = rgb("B45309"); GREEN   = rgb("15803D")
MASK_BG = rgb("F2F0EE"); MASK_DK = rgb("1B2A3E"); ROW_ALT = rgb("E8EEF5")

def i(x): return Inches(x)
def p(x): return Pt(x)

W=i(13.33); H=i(7.5); SB=i(2.55); TY=i(1.05); SB_X=i(0.32)
...
def add_rect(slide, x, y, w, h, fill_color, line_color=None):
    ...
    if line_color:
        shape.line.color.rgb = line_color; shape.line.width = p(0.4)
    else:
...
def build_slide(prs, accent, title, subtitle, header_label,
    ...
    # Masquer texte template
    add_rect(slide, i(0), i(0.60), i(6.5), i(0.55), MASK_BG)
    add_rect(slide, i(12.5), i(7.1), i(0.85), i(0.40), MASK_DK)

    # Sidebar
    sb_h = H - TY - i(0.68)
    add_rect(slide, SB_X, TY, SB, sb_h, NAVY)
    add_rect(slide, SB_X, TY, SB, i(0.07), accent)
    add_text(slide, str(count), SB_X, TY+i(0.12), SB, i(1.35),
             p(72), bold=True, color=accent, align=PP_ALIGN.CENTER)
    add_rect(slide, SB_X+i(0.2), TY+i(1.55), SB-i(0.4), i(0.02), accent)
    add_text(slide, title.upper(), SB_X, TY+i(1.62), SB, i(1.15),
             p(17), bold=True, color=WHITE, align=PP_ALIGN.CENTER)
    PY = TY + i(2.95)
    add_rect(slide, SB_X+i(0.2), PY, SB-i(0.4), i(0.52), accent)
    add_text(slide, subtitle, SB_X+i(0.2), PY, SB-i(0.4), i(0.52),
             p(10), bold=True, color=WHITE, align=PP_ALIGN.CENTER)
    add_text(slide, TODAY, SB_X, H-i(0.88), SB, i(0.22),
             p(7.5), color=SUBTEXT, align=PP_ALIGN.CENTER)

    # Table
    TTX = SB_X + SB + i(0.18)
    TTW = W - TTX - i(0.32)
    add_rect(slide, TTX, TY, TTW, H-TY-i(0.68), WHITE, BORDER)

    BH = i(0.40)
    add_rect(slide, TTX, TY, TTW, BH, accent)
    add_text(slide, f"!  {header_label}", TTX+i(0.12), TY, TTW-i(1.6), BH,
             p(9.5), bold=True, color=WHITE, align=PP_ALIGN.LEFT)
    add_rect(slide, TTX+TTW-i(1.1), TY+i(0.06), i(0.95), BH-i(0.12), WHITE, WHITE)
    add_text(slide, str(count), TTX+TTW-i(1.1), TY+i(0.06), i(0.95), BH-i(0.12),
             p(13), bold=True, color=accent, align=PP_ALIGN.CENTER)

    HY=TY+BH; HH=i(0.27); cx=TTX
    cw = [i(w) for w in col_w_in]
    for hi, hdr in enumerate(headers):
        add_rect(slide, cx, HY, cw[hi], HH, NAVY, BORDER)
        add_text(slide, hdr, cx+i(0.05), HY, cw[hi]-i(0.1), HH,
                 p(7.5), bold=True, color=AMBER, align=PP_ALIGN.LEFT)
        cx += cw[hi]

    DY=HY+HH; DH=H-DY-i(0.72)
    n=max(len(rows),1); rh=min(int(DH/n), i(0.265))
    ...
    for ri, row in enumerate(rows):
        ...
        for ci, cell in enumerate(row):
            txt=str(cell or ""); fg=TEXT; bold=False
            if ci==days_col and txt:
                fg=day_color(txt); bold=True
            add_rect(slide, cx, ry, cw[ci], rh, bg, BORDER)
            add_text(slide, txt, cx+i(0.06), ry, cw[ci]-i(0.12), rh,
                     p(7.5), bold=bold, color=fg, align=PP_ALIGN.LEFT)
            cx += cw[ci]

    add_rect(slide, TTX, DY+n*rh, TTW, i(0.025), accent)
...
def main():
    ...
    for mod, items in [("Maintenance",maint),("Composite",comp),("Avionic",avio),
        ...
        for it in items:
            if it.get('serviceability')=='Unserviceable':
                ...
                s1.append([mod,trunc(it.get('designation',''),28),
                           trunc(pn,18),it.get('serialNumber',''),
                           trunc(it.get('reason','Unserviceable'),28)])
    ...
    build_slide(prs,RED_ACC,"Other Parts","NEED TO BE C/OUT",
                "ALL UNSERVICEABLE ITEMS (ORDERED)",len(s1),
                ["MODULE","DESIGNATION","P/N","S/N","REASON"],s1,cw1)
    build_slide(prs,RED_ACC,"Life Raft","NEED TO BE C/OUT",
                "LIFE RAFT — NEED TO BE C/OUT",len(s2),
                ["P/N","S/N","REASON"],s2,cw2)
    build_slide(prs,BLUE_ACC,"EFS","DUE WITHIN 90 DAYS",
                "SERVICEABLE EFS — DUE WITHIN 90 DAYS",len(s3),
                ["H/C","DESIGNATION","P/N","S/N","NEXT 18M","NEXT 36M","DAYS LEFT"],s3,cw34,days_col=6)
    build_slide(prs,BLUE_ACC,"EFS Cylinders","DUE WITHIN 90 DAYS",
                "SERVICEABLE EFS CYLINDERS — DUE WITHIN 90 DAYS",len(s4),
                ["H/C","DESIGNATION","P/N","S/N","NEXT 18M","NEXT 60M","DAYS LEFT"],s4,cw34,days_col=6)
